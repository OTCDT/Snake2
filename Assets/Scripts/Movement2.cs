...
public class Movement2 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if (Input.GetKey(KeyCode.LeftArrow)) //When the key is pressed it will rotate the player around itself to the left
            {
                transform.RotateAround(player.transform.position, Vector3.forward, turnspeed * Time.deltaTime);
            }

            if (Input.GetKey(KeyCode.RightArrow)) //When the key is pressed it will rotate the player around itself to the right
            {
                transform.RotateAround(player.transform.position, -Vector3.forward, turnspeed * Time.deltaTime);
            }

            if (Input.GetKey(KeyCode.DownArrow)) //When the key is pressed it will slow down the player to a lower speed
            {
                speed = 9;
            }
            else
